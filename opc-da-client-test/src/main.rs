...
//!
//! ```text
//! opc-da-client-test.exe [prog_id] [host]
//! ```
//! - 无参：`opc-da-rs.Server.1` @ localhost（自建 server 本机验证，13 探针）。
//! - `<prog_id>`：指定 server（如 `Matrikon.OPC.Simulation.1` 做对比测试，定位 server vs client 问题）。
//! - `<host>`：DCOM 远程验证——连远程机（client 机也需 `/RegServer` 注册 ProgID/CLSID
//!   供 `CLSIDFromProgID` 解析，再 `CoCreateInstanceEx` 连远程 host）。
//!
//! # 当前覆盖（M8：全量验证）
//!
...
async fn main() -> anyhow::Result<()> {
    // 命令行：args[1] = server ProgID（默认 opc-da-rs.Server.1）；args[2] = host（默认 localhost）。
    // 用法：
    //   opc-da-client-test.exe                                    → opc-da-rs.Server.1 @ localhost
    //   opc-da-client-test.exe Matrikon.OPC.Simulation.1          → Matrikon @ localhost（对比测试）
    //   opc-da-client-test.exe opc-da-rs.Server.1 192.168.1.10    → opc-da-rs @ 远程 host（DCOM）
    let args: Vec<String> = std::env::args().collect();
    let prog_id = args.get(1).map_or("opc-da-rs.Server.1", String::as_str);
    let host = args.get(2).map_or("localhost", String::as_str);
    ...
    // 9. list_servers：枚举 host 上所有 OPC DA server（含 opc-da-rs / Matrikon 等，供遍历对比）。
    match client.list_servers(host).await {
        Ok(servers) => {
            println!(
                "✓ list_servers: 枚举 {} 个 server: {:?}",
                servers.len(),
                servers
            );
            passed += 1;
