...
- **Server-side OPC group leak on `add_items` failure**: the read/write handlers (`read_tag_values`, `read_tag_values_max_age`, `write_tag_value`, `write_tag_value_vqt`, `write_tag_values`) and the subscribe path (`create_group_and_advise`, shared with subscription rebuild) now `remove_group` before returning when `add_items` itself fails, instead of leaking an orphan group on the server.
